...
class _SQLDecoratedOperator(DecoratedOperator, SQLExecuteQueryOperator):
    ...
    template_fields_renderers: ClassVar[dict[str, str]] = {
        ...
        **SQLExecuteQueryOperator.template_fields.renderers,
    }

    custom_operator_name = '@task.sql'
    ...
    def __init__(
        ...
        if kwargs.pop('multiple_outputs', None):
            warnings.warn(
                f'multiple_output=True is not supported in { self.custom_operator_name} tasks. Ignoring',
                UserWarning,
                stacklevel =  3
            )
    ...
    def execute(self, context: Context) -> Any:
        ...
        if not isinstance(self.sql, str) or self.sql.strip() == '':
            raise TypeError('The returned value from the TaskFlow callable must be an non-empty string.')
